...
class CCTVDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        gt_r2 = np.reshape(gts_r[:, :, 0:self.f_ch_num], (128, 128, self.f_ch_num, 1))
        ...
        return frms, gt_w2, gt_r2, show_frms, show_gt_w, show_gt_r
...
frms, gt_w, gt_r, _, _ , _ = cctv_dataset[0] #,gt_r
...
videos, gts_w, gts_r, s_v, s_w, s_r  = next(iter(dataloaders)) #, gts_r , s_r
...
print("-"*10)
print("Create model")
model= vnet_wr.VNet(elu=False, nll=False, frm_ch=frm_ch_num, height=data_height, width=data_width)
weight_decay = 1e-4
...
for epoch in range(epochs):
    ...
    for batch_idx, (frms, gts_w, gts_r, show_frms, show_gts_w,show_gts_r) in enumerate(dataloaders): #, gts_r, show_gts_r
        ...
        #road_loss = diceL1_focal_loss(pred_road,gts_r,batch_sz)
        water_loss = dice_loss(pred_water, gts_w) + L1_loss(pred_water, gts_w)  # ,batch_sz)
        road_loss = dice_loss(pred_road, gts_r) + L1_loss(pred_road, gts_r)  # ,batch_sz)
        loss = water_loss + road_loss #L1_weight*(water_L1_loss) #+ road_loss + road_L1_loss)
